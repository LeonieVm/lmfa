...
step1 <- function(data,
  ...
  for (comparingmodels in 1:n_models) {
    ...
    for (mcluststarts in 1:n_mclust) {
      ...
      LambPsi <- updLambPsi(
        n_state, C_k, n_fact, Beta_k, Theta_k,
        residualVariance, J
      )
    ...
    loglikMulti <- as.data.frame(matrix(unlist(lapply(
      MclustResults,
      function(x) {
        x[[5]]
      }
    ...
    if (n_starts > 0) { # otherwise only mclust is used
      ...
      MultistartResults1 <- foreach(
        multistart = 1:(n_starts * 10),
        .packages = c(
          "doParallel",
          "mclust",
          "NPflow"
    ...
    cat("\n")
    cat(paste(
      "2.Iterating through the best 10 %",
      "of the start sets..."
    ))
    MultistartResults2 <- foreach(
      multistart2 = 1:(n_starts),
      .packages = c(
        "doParallel",
        "NPflow"
      ),
      ...
      M_step_parameters <- list(
        Lambda_k, # state-specific loading matrices
        lapply(Psi_k, diag)
      ) # state-specific unique variances
    ...
    while ((sum(
      abs(differenceLL) > em_tolerance, SumParameterChange > em_tolerance,
    ...
    for (i in 1:n_state) {
      if (n_fact[i] > 1) {
        ...
        rotationResultsunstandardized <- suppressWarnings(tryCatch(GPFoblq(Lambda_k[[i]], method = "oblimin", normalize = TRUE),
          error = function(ww) {
            return(list(GPFoblq(Lambda_k[[i]], method = "oblimin", normalize = FALSE), "normalizeFALSE"))
          }
        ))
    ...
    for (i in 1:n_state) {
      if (n_fact[i] > 1) {
        WarningStandardized <- suppressWarnings(tryCatch(GPFoblq(standLambda2[[i]], method = "oblimin", normalize = FALSE),
          warning = function(w) {
            return(list(GPFoblq(standLambda2[[i]], method = "oblimin", normalize = FALSE), w))
          }
        ))
    ...
    for (i in 1:n_state) {
      if (n_fact[i] > 1) {
        WarningUnstandardized <- suppressWarnings(tryCatch(GPFoblq(Lambda_k[[i]], method = "oblimin", normalize = nextNormalize),
          warning = function(w) {
            return(list(GPFoblq(Lambda_k[[i]], method = "oblimin", normalize = nextNormalize), w))
          }
        ))
